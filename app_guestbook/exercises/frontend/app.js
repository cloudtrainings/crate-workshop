...
angular.module('app', [])
  .value('apiHost', 'http://10.11.12.230:8080')
  ...
  .controller('GuestbookController', function($scope, $q, apiHost, Api, Location, objectArrayIndexOf) {
    ...
    var loadReport = function() {
        ...
        api.get().then(function(response) {
          ...
          //window.alert($scope.postonlocation.length);
    ...
    var loadComments = function(post) {
        ...
        api.get().then(function(response) {
          ...
          //window.alert($scope.comments[post.id].length + " "+ post.id);
         // window.alert($scope.comments[post.id].length + " "+ post.id + " " + $scope.comments[post.id][0].text);
        }, function(e) {
          console.warn(e);
          $scope.comments = {};
        });
